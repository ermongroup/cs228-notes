...
First, note that it is very easy to construct a $$G$$ such that $$I(G) \subseteq I(p)$$. A fully connected DAG $$G$$ is an I-map for any distribution since $$I(G) = \emptyset$$. {% include marginfigure.html id="dp1" url="assets/img/full-bayesnet.png" description="A fully connected Bayesian network over four variables. There are no independencies in this model, and it is an I-map for any distribution." %}
