...
The end result of this chapter will be a new technique called the Junction Tree (JT) algorithm{% include sidenote.html id="note-VEandJT" note="Or if you are familiar with dynamic programming (DP), you could conceptually contrast VE v.s. JT algorithm as two flavors of same technique: top-down DP v.s. bottom-up table filling. Just like in computing Fibnacci number $$F_n$$, top-down (e.g. VE) compute *just* that number, but bottom-up (e.g. JT) will create a filled table that can answer all $$F_i$$ for $$i \leq n$$. Moreover, The two-pass natrual of JT comes from the fact that the underlying DP is on bi-directional (junction) trees, while Fibnacchi numbers' relation is uni-directional linked list. " %} ; this algorithm will first execute two runs of the VE algorithm to initialize a particular data structure holding a set of pre-computed factors. Once the structure is initialized, it will be used to answer marginal queries in $$O(1)$$ time.
