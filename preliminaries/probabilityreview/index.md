...
More formally, a random variable $$X$$ is a function $$X : \Omega \to \Re$$. Typically, we denote random variables using upper case letters $$X(\omega)$$ or more simply $$X$$ (where the dependence on the random outcome $$\omega$$ is implied). We denote the value that a random variable may take on using lower case letters $$x$$. Thus, $$X = x$$ means that we assign the value $$x \in \Re$$ to the random variable $$X$$.
