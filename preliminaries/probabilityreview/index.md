...
More formally, a random variable $$X$$ is a function $$X : \Omega \to \Re$$. Typically, we will denote random variables using upper case letters $$X(\omega)$$ or more simply $$X$$ (where the dependence on the random outcome $$\omega$$ is implied). We will denote the value that a random variable may take on using lower case letters $$x$$. Thus, $$X = x$$ means that we are assigning the value $$x \in \Re$$ to the random variable $$X$$.
...
Suppose that we have two discrete random variables $$X, Y$$ and $$g : \Re^2 \to \Re$$ is a function of these two random variables. Then the expected value of $$g$$ is defined in the following way,
